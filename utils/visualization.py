...
def save_hist(writer, mode_tag, disp_error, global_step, bins=np.arange(-3, 3, 1/200) + 0.5 * 1/200):  # bins=np.arange(-5, 5, 1/100)
    disp_error[disp_error < bins[0]] = bins[0]
    disp_error[disp_error > bins[-1]] = bins[-1]

    writer.add_histogram(mode_tag, disp_error, global_step,  bins=bins)
